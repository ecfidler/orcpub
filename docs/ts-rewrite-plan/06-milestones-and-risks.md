...
|---|-----------|----------|-------|------|
| M0 | Reference & fixtures | Old app running (Plan Set 1 doc 01); golden characters incl. homebrew and legacy cases; captured `GET /dnd/5e/characters/:id` payloads; community `.orcbrew` packs; built-value dumps | Fixture repo committed | ~1 week |
| M1 | Engine library | Vendored source; shadow-cljs build; base facade (`evaluate`, mutations, `importCharacter`); `warlock_test` + `character_test` round-trips ported | Golden characters evaluate identically in TS tests | ~2–3 weeks |
| M2 | Local-first viewer | App scaffold; import a captured character; read-only sheet from `evaluate`; IndexedDB storage; native export | First visible proof; Playwright smoke test | ~2 weeks |
| M3 | Homebrew engine path | De-re-framed `buildTemplate`; `parseOrcbrew`/export via facade; both fixtures + community packs import; homebrew golden character matches | Lossless and old-spec-acceptance tests green | ~2–3 weeks |
...
- **Why vendor the engine**: the new app needs four small patches
  (doc 02) and must not inherit Leiningen or the re-frame UI.
...
| Upstream engine fixes drift away from the vendored copy | Medium | Resync script + `patches/` kept tiny; golden tests catch behavior changes |
| Bundle size (~2 MB of content source) | Certain | `:advanced`; async chunk; exclude `random.cljc`, `templates/`, and serve monsters as JSON |
| Non-SRD leakage (name tables) into a public repo | Low | Excluded from vendoring by list |
